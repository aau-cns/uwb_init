...
namespace uwb_init
{
OptWpsGenerator::OptWpsGenerator(const std::shared_ptr<Logger> logger, const PlannerOptions& planner_options)
    : logger_(std::move(logger))
{
    ...
    std::stringstream ss;
    ss << "OptWpsGenerator options: C_e = "
       << planner_options_.C_e_.transpose() << '\n'
       << "OptWpsGenerator options: C_v = "
       << planner_options_.C_v_.transpose() << '\n';
...
void OptWpsGenerator::sortrows(Eigen::MatrixXd& A_nx3, int idx)
{
    ...
    std::sort(vec.begin(), vec.end(), [idx](Eigen::VectorXd const& t1, Eigen::VectorXd const& t2) {
...
double OptWpsGenerator::dp(Eigen::MatrixXd P, Eigen::MatrixXd UWBs)
{
    ...
    Eigen::VectorXd ones(P.rows()); ones.setOnes();
...
double OptWpsGenerator::fitness_calculation(Eigen::MatrixXd chromosome, tens R, const Eigen::MatrixXd& UWBs, uint l_c, const Eigen::VectorXd& p_k)
{
...
Eigen::MatrixXd OptWpsGenerator::genrate_cube(double xs_2, double ys_2, double zs_2)
{
    ...
    Eigen::VectorXd ones(5); ones.setOnes();
...
{
    Eigen::VectorXd temp;
    Eigen::MatrixXd xyz = genrate_cube(planner_options_.x_s_ / 2,
                                planner_options_.y_s_ / 2,
                                planner_options_.z_s_ / 2);
    ...
    for (uint i = 0; i < planner_options_.z_n_; i++) {
        for (uint j = 0; j < planner_options_.y_n_; j++) {
            for (uint k = 0; k < planner_options_.x_n_; k++) {
                temp = Eigen::VectorXd::LinSpaced(planner_options_.cell_len_, xyz(0, 0)
                                           + planner_options_.x_s_ * (k), xyz(0, 1)
                                           + planner_options_.x_s_ * (k));
                R[c].row(0) << temp.transpose();
                temp = Eigen::VectorXd::LinSpaced(planner_options_.cell_len_, xyz(2, 0)
                                           + planner_options_.y_s_ * (j), xyz(2, 3)
                                           + planner_options_.y_s_ * (j));
                R[c].row(1) << temp.transpose();
                temp = Eigen::VectorXd::LinSpaced(planner_options_.cell_len_, xyz(4, 0)
                                           + planner_options_.z_s_ * (i), xyz(5, 0)
                                           + planner_options_.z_s_ * (i));
...
bool OptWpsGenerator::selection(tens& selcted_chrom_for_reprodoction, tens& elit, tens& selcted_chrom_for_mutaion, tens pop, Eigen::VectorXd Fit, uint c_n, uint m_n, uint e_n)
{
    bool flag_ext = false;
    ...
    if ((F_max - F_min) < 0.001) {
        ...
        Eigen::VectorXd U = repelem(Eigen::VectorXd::LinSpaced(planner_options_.pop_size_, 0, planner_options_.pop_size_ - 1), N);
...
void OptWpsGenerator::crossover(tens& selcted_chrom_for_reprodoction, uint l_c, uint c_n)
{
    ...
    for (uint i = 0; i < c_n; i = i + 2) {
        ...
        selcted_chrom_for_reprodoction[i] << temp_i(Eigen::seq(0, cpoint, 1), Eigen::all), temp_ii(Eigen::seq(cpoint + 1, l_c - 1, 1), Eigen::all);
        selcted_chrom_for_reprodoction[i + 1] << temp_ii(Eigen::seq(0, cpoint, 1), Eigen::all), temp_i(Eigen::seq(cpoint + 1, l_c - 1, 1), Eigen::all);
    }
}
