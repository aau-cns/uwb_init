    ...
    : logger_(std::make_shared<Logger>(level)), params_(params)
{
    ...
    case UwbInitOptions::InitMethod::SINGLE:
        ...
        case UwbInitOptions::InitVariables::NO_BIAS:
            ls_problem = std::bind(&UwbInitializer::ls_single_no_bias, this, std::placeholders::_1,
    ...
    case UwbInitOptions::InitMethod::DOUBLE:
        ...
        {
        case UwbInitOptions::InitVariables::NO_BIAS:
            ls_problem = std::bind(&UwbInitializer::ls_double_no_bias, this, std::placeholders::_1,
                                   std::placeholders::_2, std::placeholders::_3);
            break;
...
void UwbInitializer::feed_uwb(const double timestamp, const std::vector<UwbData> uwb_measurements)
{
    ...
    for (uint i = 0; i < uwb_measurements.size(); ++i)
    {
        ...
        if (uwb_measurements[i].valid_ && uwb_measurements[i].distance_ > 0.0)
        {
            ...
            uwb_data_buffer_[uwb_measurements[i].id_].
                    push_back(timestamp, uwb_measurements[i]);
        }
        else
        {
...
bool UwbInitializer::ls_single_const_bias(const TimedBuffer<UwbData>& uwb_data, Eigen::MatrixXd& A,
                                          Eigen::VectorXd& b)
{
    // Const bias single:
    // [    0   ,     1   ,    2    ,  3 ,      4            ]
    ...
    for (uint i = 0; i < uwb_data.size(); ++i)
    {
        ...
        row << -2 * p_UinG.x(), -2 * p_UinG.y(), -2 * p_UinG.z(),
                2 * uwb_data[i].second.distance_, 1;
...
bool UwbInitializer::ls_single_no_bias(const TimedBuffer<UwbData>& uwb_data, Eigen::MatrixXd& A,
                                       Eigen::VectorXd& b)
{
    // Unbiased single:
    // [    0    ,    1   ,    2   ,        3       ]
...
bool UwbInitializer::ls_double_const_bias(const TimedBuffer<UwbData>& uwb_data, Eigen::MatrixXd& A,
                                          Eigen::VectorXd& b)
{
    // Const bias double:
    // [    0   ,     1   ,    2    ,  3 ]
    ...
    for (uint i = 0; i < uwb_data.size(); ++i)
    {
        ...
        row << -(p_UinG.x() - p_UinG_pivot.x()), -(p_UinG.y() - p_UinG_pivot.y()),
                -(p_UinG.z() - p_UinG_pivot.z()), (uwb_data[i].second.distance_ - uwb_pivot);
...
bool UwbInitializer::ls_double_no_bias(const TimedBuffer<UwbData>& uwb_data, Eigen::MatrixXd& A,
                                       Eigen::VectorXd& b)
{
    // Unbiased double:
    // [    0    ,    1   ,    2    ]
