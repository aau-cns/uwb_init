...
//                     std::to_string(uwb_measurements[i].distance) + " from anchor " +
//                     std::to_string(uwb_measurements[i].id));
//     }
//   }
// }
...
//   {
//     logger_->err("Anchor[" + std::to_string(anchor_id) + "]: Condition number greater than " +
//                  std::to_string(params_.max_cond_num));
//     new_uwb_anchor.initialized = false;
//     return false;
//   }
...
//   {
//     // Refine unsuccessful
//     logger_->err("Anchor[" + std::to_string(anchor.id) +
...
//     double tolerance =
//         std::numeric_limits<double>::epsilon() * std::max(J.cols(), J.rows()) *
...
//     {
//       logger_->info("Anchor[" + std::to_string(anchor.id) + "]: Step norm is less than " +
...
//     {
//       logger_->info("Anchor[" + std::to_string(anchor.id) + "]: Residual is less than " +
...
//     {
//       logger_->warn("Anchor[" + std::to_string(anchor.id) + "]: Maximum number of iterations reached (" +
...
// bool UwbInitializer::ls_single_full_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                          Eigen::VectorXd& b)
// {
//   // Full bias single:
//   // [      0     ,       1     ,       2     ,  3 , 4,          5              ]
...
//   if (!(params_.b_perform_regularization))
//     return true;

//   // Check to have more than 6 rows in the coefficient matrix
//   if (!(A.rows() > A.cols()))
//   {
//     logger_->warn("UwbInitializer::ls_single_full_bias(): Regularization can not be performed (coeffs matrix has less
//                   "
//                   "than " +
...
// bool UwbInitializer::ls_single_dist_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                          Eigen::VectorXd& b)
// {
//   // Dist bias single:
//   // [      0     ,       1     ,       2     ,  3 ,         4         ]
...
//     Eigen::VectorXd row(A.cols());
//     row << -2 * closest_p_UinG.x(), -2 * closest_p_UinG.y(), -2 * closest_p_UinG.z(),
//         std::pow(closest_p_UinG.norm(), 2), 1;
//     A.row(i) = row.transpose();
//     b(i) = std::pow(uwb_data.at(i).second.distance, 2);
//   }
...
//   if (!(A.rows() > A.cols()))
//   {
//     logger_->warn("UwbInitializer::ls_single_dist_bias(): Regularization can not be performed (coeffs matrix has less
//                   "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
...
// bool UwbInitializer::ls_single_const_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                           Eigen::VectorXd& b)
// {
//   // Const bias single:
//   // [    0   ,     1   ,    2    ,  3 ,      4            ]
...
//     Eigen::VectorXd row(A.cols());
//     row << -2 * closest_p_UinG.x(), -2 * closest_p_UinG.y(), -2 * closest_p_UinG.z(),
//         2 * uwb_data.at(i).second.distance, 1;
//     A.row(i) = row.transpose();
//     b(i) = std::pow(uwb_data.at(i).second.distance, 2);
//   }
...
//   if (!(A.rows() > A.cols()))
//   {
//     logger_->warn("UwbInitializer::ls_single_const_bias(): Regularization can not be performed (coeffs matrix has
//                   less "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
...
// bool UwbInitializer::ls_single_no_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                        Eigen::VectorXd& b)
// {
//   // Unbiased single:
//   // [    0    ,    1   ,    2   ,        3       ]
...
//   if (!(A.rows() > 4))
//   {
//     logger_->warn("UwbInitializer::ls_single_no_bias(): Regularization can not be performed (coeffs matrix has less "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
//   }
...
// bool UwbInitializer::ls_double_full_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                          Eigen::VectorXd& b)
// {
//   // Full bias double:
//   // [      0     ,       1     ,       2     ,  3 , 4]
...
//   {
//     // baseline check
//     double diff = uwb_data.at(i).second.distance - uwb_data.at(i + params_.meas_baseline_idx_).second.distance;
...
//         buffer_p_UinG_.get_closest(uwb_data.at(i + params_.meas_baseline_idx_).first - params_.t_pose_diff,
//                                    closest_p_UinG2))
//     {
//       Eigen::VectorXd row(n_params);
//       row << -2 * (closest_p_UinG1.x() - closest_p_UinG2.x()), -2 * (closest_p_UinG1.y() - closest_p_UinG2.y()),
//           -2 * (closest_p_UinG1.z() - closest_p_UinG2.z()),
//           std::pow(closest_p_UinG1.norm(), 2) - std::pow(closest_p_UinG2.norm(), 2), 2 * diff;
...
//   {
//     logger_->warn("UwbInitializer::ls_double_full_bias(): Regularization can not be performed (coeffs matrix has less
//                   "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
...
// bool UwbInitializer::ls_double_dist_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                          Eigen::VectorXd& b)
// {
//   // Dist bias double:
//   // [      0     ,       1     ,       2     ,  3 ]
...
//   if (!(coeffs_vec.size() > n_params * n_params))
//   {
//     logger_->warn("UwbInitializer::ls_double_dist_bias(): Regularization can not be performed (coeffs matrix has less
//                   "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
...
// bool UwbInitializer::ls_double_const_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                           Eigen::VectorXd& b)
// {
//   // Const bias double:
//   // [    0   ,     1   ,    2    ,  3 ]
...
//   if (!(coeffs_vec.size() > n_params * n_params))
//   {
//     logger_->warn("UwbInitializer::ls_double_const_bias(): Regularization can not be performed (coeffs matrix has
//                   less "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
...
// bool UwbInitializer::ls_double_no_bias(std::deque<std::pair<double, UwbData>>& uwb_data, Eigen::MatrixXd& A,
//                                        Eigen::VectorXd& b)
// {
//   // Unbiased double:
//   // [    0    ,    1   ,    2    ]
...
//   if (!(coeffs_vec.size() > n_params * n_params))
//   {
//     logger_->warn("UwbInitializer::ls_double_no_bias(): Regularization can not be performed (coeffs matrix has less "
//                   "than " +
//                   std::to_string(A.cols()) + " rows)");
//     return true;
//   }
